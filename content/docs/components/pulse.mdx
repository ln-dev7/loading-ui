...
The ring uses `border-current`, so semantic `text-*` utilities tint the stroke predictably.
...
Breathing speed follows `--tw-animation-duration` (for example via `animation-duration-*` utilities).
